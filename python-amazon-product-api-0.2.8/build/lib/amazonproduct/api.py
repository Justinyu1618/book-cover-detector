...
class GZipHandler(urllib.request.BaseHandler):
    ...
    def http_response(self, req, resp):
        if resp.headers.get('content-encoding') == 'gzip':
            ...
            old = resp
            resp = urllib.request.addinfourl(gz, old.headers, old.url)
            resp.msg = old.msg
...
class API (object):
    ...
    *associate ID*. Create the file ``~/.amazon-product-api`` and add the
    ...
    def _build_url(self, **qargs):
        ...
        signature = quote(
            b64encode(hmac.new(codecs.encode(self.secret_key) or '', msg.encode('utf-8'), sha256).digest()))
    ...
    def _fetch(self, url):
        ...
        opener = urllib.request.build_opener(handler, GZipHandler())
